...
export const Pagination: FC<PaginationPropsType> = (props) => {
  ...
  const pages = Array.from({ length: pagesLength }, (_, i) => i + 1).filter(
    (item) => item !== lastPage && item !== 1 && Math.abs(item - currentPage) <= 2
  );
  ...
  return (
    <div className={styles.paginationButtons}>
      ...
      <button type="button" disabled={loading || currentPage === lastPage} onClick={() => onSelectPage(lastPage)}>
        {lastPage}
      </button>
    </div>
  );
